...
export default class extends window.Controller {
    ...
    connect() {
        ...
        const tools = this.createDynamicTools(blocksData);
    ...
    createDynamicTools(blocks) {
        const tools = {};
        
        blocks.forEach(block => {
            try {
                const content = JSON.parse(block.content);
                const toolName = block.slug;
                ...
                tools[toolName] = {
                    ...
                    // Переопределяем toolbox для каждого инструмента
                    toolbox: {
                        title: block.name, // Используем реальное название
                        icon: `
                ...
                console.error(`Ошибка парсинга content для блока ${block.name}:`, e);
            }
        });
